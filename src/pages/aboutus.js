...
export default function AboutUs() {
    ...
    let Products=[
        {
            heading:"Sourcing",
            content:"Spot top talent before they’re even looking."
        },
        {
            heading:"Screening",
            content:"Filter brilliance from the noise—instantly."
        },
        {
            heading:"Interview",
            content:"Conversations that qualify. Insights that close."
        },
        {
            heading:"Background verification",
            content:"Trust, verified—before the first handshake."
        }
    ]
    return (
        <>
            ...
            <section className="relative md:py-24 py-16">
                ...
                <div className="container relative">
                    ...
                    /> */}
                        <div className="">
                            ...
                            <p className="text-slate-400 max-w-xl">"Usually, hiring feels like a tedious chore for many teams, but since we have come up with the AI interview platform, the enthusiasm and efficiency in our recruitment process will skyrocket like never before."</p>

                            <ul className="list-none text-slate-400 mt-4">
                                <li className="mb-2 flex "><FiCheckCircle className="text-amber-400 h-5 w-5 me-2" /> Let AI quickly analyze resumes and interview responses, saving hours of manual review.</li>
                                <li className="mb-2 flex "><FiCheckCircle className="text-amber-400 h-5 w-5 me-2" /> Access instant analytics and candidate scoring to make smarter hiring choices faster.</li>
                                <li className="mb-2 flex "><FiCheckCircle className="text-amber-400 h-5 w-5 me-2" /> AI adapts to your hiring volume, whether you’re filling one position or hundreds.</li>
                                <li className="mb-2 flex "><FiCheckCircle className="text-amber-400 h-5 w-5 me-2" /> Assessment that candidates ❤ to take.</li>
                            </ul>
                            {/* //have removed items-center in classname */}
                            {/* <div className="mt-4">
                ...
                <div className="container relative md:mt-24 mt-16">
                    <h1 className="mb-4 md:text-3xl md:leading-normal text-2xl leading-normal font-semibold">Products:</h1>
                    <div className="lg:flex justify-center">

                        <div className="lg:w-4/5">
                            <ul className="flex justify-center text-center p-2 bg-white dark:bg-slate-900 shadow dark:shadow-gray-800 rounded-md overflow-x-auto whitespace-nowrap">
                                {Products.map((title, index) => (
                                    <li
                                        role="presentation"
                                        key={index}
                                        className="p-2 min-w-[280px] flex-shrink-0"
                                    >
                                        <button
                                            className={`${activeIndex === index
                                                ? "text-white bg-amber-400"
                                                : "hover:text-amber-400 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-slate-800"
                                                } px-5 py-3 text-start rounded-md duration-500 w-full`}
                                            onClick={() => setActiveIndex(index)}
                                        >
                                            <h5 className="text-base font-semibold">{title.heading}</h5>
                                            <p className="text-sm mt-1">{title.content}</p>
                                        </button>
                                    </li>
                                ))}
                            </ul>
